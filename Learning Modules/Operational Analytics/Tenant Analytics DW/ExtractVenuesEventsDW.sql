...
@job_name='ExtractVenuesEventsDW',
...
@output_database_name='tenantanalytics-dw',
@output_table_name='VenuesEventsRawData'

-- checking parent job status
SELECT * FROM [jobs].[job_executions] 
WHERE job_name = 'ExtractVenuesEventsDW' and step_id IS NULL

-- checking job status
SELECT * FROM [jobs].[job_executions] 
WHERE job_name = 'ExtractVenuesEventsDW'

-- Cleanup
--EXEC [jobs].[sp_delete_job] 'ExtractVenuesEventsDW'
--EXEC [jobs].[sp_start_job] 'ExtractVenuesEventsDW'
