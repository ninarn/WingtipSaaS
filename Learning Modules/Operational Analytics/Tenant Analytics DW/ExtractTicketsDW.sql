...
WHERE job_name = 'ExtractTicketsDW' and step_id IS NULL

--View all execution status
SELECT * FROM [jobs].[job_executions] 
WHERE job_name = 'ExtractTicketsDW'

-- Cleanup
--EXEC [jobs].[sp_delete_job] 'ExtractTicketsDW'
--EXEC [jobs].[sp_delete_target_group] 'TenantGroup'
--EXEC [jobs].[sp_start_job] 'ExtractTicketsDW'